...
for i in range(horizontal_pos):
	for j in range(vertical_pos):
		for k in range(channels[0]):
			input('Press if ready for hpos: {}, vpos:{}, channel: {}'.format(i, j, k))
...
for i in range(horizontal_pos):
	for j in range(vertical_pos):
		for k in range(channels[0]):
			raw_data_reshape[i, 0, k*samples_per_chirp:(k+1)*samples_per_chirp, j] = list(raw_data[index])
			index += 1
...
file = open(os.path.join(outdir, 'box.pkl'), 'wb')
